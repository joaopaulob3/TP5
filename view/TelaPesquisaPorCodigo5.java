...
public class TelaPesquisaPorCodigo5 implements ActionListener {
	//Atributos
	...
	//Construtor
	public TelaPesquisaPorCodigo5(JFrame listagemPerfume, JFrame frmTelaCompra, int opcao, Cliente cliente) {
		//Configura os componentes da JFrame da tela de pesquisa pelo código
		this.listagemPerfume = listagemPerfume;
		this.frmTelaCompra = frmTelaCompra;
		this.opcao = opcao;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (botao == this.getBtnPesquisar()) {
			...
			case 1:
				...
				if (perfume != null) {
					this.getFrmPesquisa().dispose();
					new TelaInformarQuantidade(this.getListagemPerfume() ,this.getfrmTelaCompra(), this.getOpcao(), perfume, this.getCliente());
				} else {
					...
					this.getFrmPesquisa().dispose();
					this.getfrmTelaCompra().setLocationRelativeTo(null);
					this.getfrmTelaCompra().setVisible(true);
				}
				break;
			case 2:
	...
	public JFrame getfrmTelaCompra() {
		return this.frmTelaCompra;
	}
